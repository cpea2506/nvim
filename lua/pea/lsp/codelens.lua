        ...
        if self.row_version[row] ~= self.version then
            for client_id, state in pairs(self.client_state) do
                ...
                if not lenses then
                    ...
                    ---@type [string, string][]
                    local virt_text = {
                        { string.rep(" ", indent), "LspCodeLensSeparator" },
                    }
                    ...
                    local had_extmark = #api.nvim_buf_get_extmarks(bufnr, namespace, { row, 0 }, { row, -1 }, {}) > 0
...
function M.get(filter)
    if type(filter) == "number" then
        vim.deprecate("vim.lsp.codelens.get(bufnr)", "vim.lsp.codelens.get({ bufnr = bufnr })", "0.13.0")
